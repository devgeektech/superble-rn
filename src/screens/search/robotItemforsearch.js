...
export default class RobotItemForSearch extends React.Component {
  ...
  productClick(event) {
    this.props.navigate('Product', { item: event, screenName: 'search' });
  }
        ...
        {(this.recordContent.type == 'product' && this.state.status !== false) &&
          <View style={{ marginLeft: 2, paddingHorizontal: 15, paddingBottom: 20 }}>
            <View style={{ flexWrap: 'wrap', flexDirection: 'row' }}>
              {temparr.map((i, v) =>
                <TouchableOpacity key={'tags' + v} onPress={() => this.onTagClicked(i)}>
                  <Text key={v} style={styles.tags}>{'#' + i.name} </Text>
                </TouchableOpacity>
              )}
            </View>
          </View>
